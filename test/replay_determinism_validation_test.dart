...
void main() {
  ...
  group('Replay Determinism Validation', () {
    ...
    test('1. Sequential ordered replay (100x -> same checksum)', () async {
      final List<ProjectionEvent> stream = List.generate(
        50,
        (i) => ProjectionEvent(
          eventSequence: i + 1,
    ...
    test(
      '2. Out-of-order replay rejection (sequence gap triggers rebuild)',
      () async {
        await cursorManager.updateCursor(
          const ReplayCursor(
            projectionId: 'menu_proj_1',
    ...
    test(
      '3. Duplicate event replay rejection (stale sequence ignored safely)',
      () async {
        await cursorManager.updateCursor(
          const ReplayCursor(
            projectionId: 'menu_proj_1',
