...
import 'package:orderlli_admin/features/menu/runtime/merge_policy_registry.dart';
...
void main() {
  ...
  group('OCC Convergence Validation', () {
    ...
    test('1. Deterministic merge output (same conflict replayed -> identical merge result)', () {
      ...
      for (int i = 0; i < 50; i++) {
        ...
        final mergedItemsStr = result.reconciledState.items.map((i) => '\${i.id}-\${i.price.amountInCents}-\${i.isAvailable}').join('|');
    ...
    test('2. Tombstone precedence enforced (deleted entities never resurrect)', () {
      ...
      final serverAuthoritative = baseSnapshot.copyWith(
        items: [
          baseSnapshot.items[0].copyWith(price: const Money(amountInCents: 1500)),
          baseSnapshot.items[1],
        ],
        snapshotVersion: '11',
      );
      ...
      expect(result.reconciledState.items.any((i) => i.id == 'item_burger'), isFalse);
    ...
    test('4. Conflict envelopes generated correctly & Manual review conflicts enter CONFLICT_REQUIRES_REVIEW', () {
    ...
    test('5. Replay after conflict remains deterministic & rebuilds converge correctly', () {
      ...
      expect(result1.envelope!.conflictFields, result2.envelope!.conflictFields);
      expect(result1.reconciledState.items[0].price.amountInCents, result2.reconciledState.items[0].price.amountInCents);
