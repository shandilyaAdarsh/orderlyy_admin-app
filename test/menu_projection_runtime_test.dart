...
void main() {
  ...
  group('ProjectionReconciliation Tests', () {
    test(
      'reconcile overrides isAvailable correctly based on availabilityOverlay',
      () {
        ...
        final burger = reconciled.items.firstWhere(
          (i) => i.id == 'item_burger',
        );
  ...
  group('ProjectionIntegrity Tests', () {
    ...
    test('invalid modifier group references return validation errors', () {
      ...
      expect(
        result.errors[0],
        contains('references non-existent modifier group ID'),
      );
    });
  });
  ...
  group('ModifierResolver Tests', () {
    test('resolveGroupsForItem resolves list of modifier groups correctly', () {
      final burger = baseSnapshot.items.firstWhere(
        (i) => i.id == 'item_burger',
      );
    ...
    test(
      'validateSelection checks if selected option exists in allowed groups',
      () {
        final burger = baseSnapshot.items.firstWhere(
          (i) => i.id == 'item_burger',
        );
  ...
  group('SnapshotMigration Tests', () {
    ...
    test(
      'verifyAndMigrate returns true and does not clear cache if version matches',
      () async {
    ...
    test(
      'verifyAndMigrate clears cache and returns false if version mismatches',
      () async {
  ...
  group('OccConflictResolver Tests', () {
    ...
    test(
      'resolveSnapshotConflict returns local changes if server is at base version',
      () {
    ...
    test(
      'resolveSnapshotConflict attempts structural merge and succeeds if no overlapping changes',
      () {
        final localOptimistic = baseSnapshot.copyWith(
          items: [
            baseSnapshot.items[0].copyWith(isAvailable: false),
